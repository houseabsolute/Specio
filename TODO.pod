...
=head1 De-Moose all the code

This is started in the de-moose branch. Also see de-moose-ify (doh, naming)
for an older start on this.

Basically, rewrite the code to not use Moose or Moo. Using Role::Tiny is okay
though.

As a corrolary, this means getting rid of Throwable, since that uses Moo. We
could basically embed a copy/rewrite of Throwable that uses POPO, since
Throwable is quite minimal.
